...
LogicalResult MatchConvFormat(stablehlo::ConvolutionOp op) {
...
DenseIntElementsAttr TransposeConvFilterValue(
 ...
 private:
  ...
  TFL::QConstOp GetBiasOpAndUpdateQuantizedFilterConstant(
    ...
    const DenseIntElementsAttr new_filter_value_attr =
        TransposeConvFilterValue(filter_op->getLoc(), rewriter,
...
class RewriteHybridQuantizedConvolutionOp
 ...
 public:
  ...
  void rewrite(stablehlo::ConvolutionOp op,
    ...
    stablehlo::ConvDimensionNumbersAttr new_dimension_numbers =
        GetNewDimensionNumbers(rewriter.getContext(), op.getDimensionNumbers(),
                               is_depthwise);
 ...
 private:
  ...
  Type GetNewWeightQuantizedType(MLIRContext* context, Location location,
    ...
    if (element_type.isa<UniformQuantizedPerAxisType>()) {
      ...
      auto new_filter_quantized_type = CreateI8F32UniformQuantizedPerAxisType(
          ...
          per_axis_type.getZeroPoints(), is_depthwise ? 3 : 0,
          /*narrow_range=*/true);
  ...
  // Returns new dimension numbers for convolution after transposing weights.
  stablehlo::ConvDimensionNumbersAttr GetNewDimensionNumbers(
      MLIRContext* context,
      stablehlo::ConvDimensionNumbersAttr dimension_numbers,
      bool is_depthwise) const {
    int64_t kernel_input_feature_dim = is_depthwise ? 0 : 3;
    int64_t kernel_output_feature_dim = is_depthwise ? 3 : 0;
