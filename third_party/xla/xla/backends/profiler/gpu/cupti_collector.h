...
class CuptiTraceCollector {
 public:
  ...
  virtual AnnotationMap* annotation_map() = 0;
  ...
  virtual void AppendCallbackAnnotationEvent(uint32_t device_id,
                                             ...
                                             absl::string_view annotation,
                                             absl::string_view nvtx_range) = 0;

  // Return the last event in per-thread call back event buffer or nullptr.
  virtual CuptiTracerEvent* LastCallbackEvent() = 0;
  ...
  virtual void GatherAllCallbackAnnotationsAndEvents() = 0;
  ...
  virtual void ClearAllAnnotatedEvents() = 0;

  // Right before profiling, setting options which impact per-thread callback
  // events collections.
  virtual void PrepareOptionSettings() = 0;
  ...
  virtual void CacheActivityBuffer(uint8_t* buffer, size_t size) = 0;
