...
using R2F32Buffer = typename ffi::BufferR2<PrimitiveType::F32>;
...
static absl::Status FfiR2F32ReduceSum(R2F32Buffer in, R0F32Buffer out) {
  auto in_data = DataPointer<float>(in);
  auto out_data = DataPointer<float>(out);

  // Calculate the total size of the vector
  const auto size = std::accumulate(in.dimensions.begin(), in.dimensions.end(),
                                    1, std::multiplies<int>());

  // Calculate the sum of the vector
  *out_data = std::accumulate(in_data, in_data + size, 0.0f);

  return absl::OkStatus();
}

XLA_FFI_DEFINE_HANDLER(kFfiR2F32ReduceSum, FfiR2F32ReduceSum,
                       ffi::Ffi::Bind()
                           .Arg<R2F32Buffer>()  // in
                           .Arg<R0F32Buffer>()  // out
);

XLA_FFI_REGISTER_HANDLER(ffi::GetXlaFfiApi(), "__xla_test$$FfiR2F32ReduceSum",
                         "Host", kFfiR2F32ReduceSum);

static absl::Status FfiR2F32Add1ToValues(R2F32Buffer in, R2F32Buffer out) {
  auto in_data = DataPointer<float>(in);
  auto out_data = DataPointer<float>(out);

  // Calculate and verify the total size of the vector
  const auto in_size = std::accumulate(
      in.dimensions.begin(), in.dimensions.end(), 1, std::multiplies<int>());
  const auto out_size = std::accumulate(
      out.dimensions.begin(), out.dimensions.end(), 1, std::multiplies<int>());
  if (in_size != out_size) {
...
XLA_FFI_DEFINE_HANDLER(kFfiR2F32Add1ToValues, FfiR2F32Add1ToValues,
                       ffi::Ffi::Bind()
                           .Arg<R2F32Buffer>()  // in
                           .Arg<R2F32Buffer>()  // out
);
...
XLA_FFI_REGISTER_HANDLER(ffi::GetXlaFfiApi(),
                         "__xla_test$$FfiR2F32Add1ToValues", "Host",
                         kFfiR2F32Add1ToValues);
...
XLA_TEST_F(FfiCustomCallTest, FfiUnknownTarget) {
  ...
  auto status = BuildAndExecute({}).status();
  EXPECT_NE(status.code(), absl::StatusCode::kUnimplemented);
}
...
XLA_TEST_F(FfiCustomCallTest, FfiReportsFailure) {
  auto constant = builder_.AddInstruction(
      HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(42.0f)));
  builder_.AddInstruction(HloInstruction::CreateCustomCall(
      ShapeUtil::MakeShape(F32, {}), {constant}, "__xla_test$$always_fail",
...
XLA_TEST_F(FfiCustomCallTest, FfiReportsFirstFailure) {
  ...
  auto constant_2 = builder_.AddInstruction(
      HloInstruction::CreateConstant(LiteralUtil::CreateR0<float>(2.0f)));
  auto res_1 = builder_.AddInstruction(HloInstruction::CreateCustomCall(
      ShapeUtil::MakeShape(F32, {}), {constant_1}, "__xla_test$$always_fail",
      /*opaque=*/"", CustomCallApiVersion::API_VERSION_TYPED_FFI));
  auto res_2 = builder_.AddInstruction(HloInstruction::CreateCustomCall(
      ShapeUtil::MakeShape(F32, {}), {constant_2}, "__xla_test$$always_fail",
      /*opaque=*/"", CustomCallApiVersion::API_VERSION_TYPED_FFI));
  builder_.AddInstruction(HloInstruction::CreateBinary(
      ShapeUtil::MakeShape(F32, {}), HloOpcode::kAdd, res_1, res_2));
...
XLA_TEST_F(FfiCustomCallTest, FfiTransitiveCustomCallReportsFirstFailure) {
  const char* const kModuleStr = R"(
    ...
    sub {
      p0 = f32[] parameter(0)
      ROOT custom-call = f32[] custom-call(f32[] %p0), custom_call_target="__xla_test$$always_fail", api_version=API_VERSION_TYPED_FFI
    }
    ENTRY test {
      c0 = f32[] constant(1.0)
      c1 = f32[] constant(2.0)
...
XLA_TEST_F(FfiCustomCallTest, FfiWrongTypeOfArguments) {
  ...
  auto constant = builder_.AddInstruction(
      HloInstruction::CreateConstant(LiteralUtil::CreateR2FromArray2D(array)));
  builder_.AddInstruction(HloInstruction::CreateCustomCall(
      r2f32_, {constant}, "__xla_test$$always_succeed", "",
      /*api_version=*/CustomCallApiVersion::API_VERSION_TYPED_FFI));
...
XLA_TEST_F(FfiCustomCallTest, FfiUsedInOtherComputations) {
  ...
  auto incremented = builder_.AddInstruction(HloInstruction::CreateCustomCall(
      ...
      "__xla_test$$FfiR2F32Add1ToValues",
      /*opaque=*/"",
      /*api_version=*/CustomCallApiVersion::API_VERSION_TYPED_FFI));
  auto incremented_again =
      builder_.AddInstruction(HloInstruction::CreateCustomCall(
...
XLA_TEST_F(FfiCustomCallTest, FfiInputAndOutputLayoutDiffer) {
  ...
  builder_.AddInstruction(HloInstruction::CreateCustomCall(
      r2f32_, {input}, "__xla_test$$FfiR2F32Add1ToValues", /*opaque=*/"",
      /*api_version=*/CustomCallApiVersion::API_VERSION_TYPED_FFI));
...
XLA_TEST_F(FfiCustomCallTest, FfiLayoutConstrained) {
  ...
  const Shape& r2f32_dim0_major =
      ShapeUtil::MakeShapeWithDenseLayout(F32, {2, 2}, {1, 0});
  auto custom_call = builder_.AddInstruction(HloInstruction::CreateCustomCall(
      r2f32_dim0_major, {input}, "__xla_test$$FfiR2F32Add1ToValues",
